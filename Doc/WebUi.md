...
- 优先通过 Steam Overlay 的内置网页浏览器打开；Steam 未初始化或叠加层不可用时，回退到系统默认浏览器，Edge、Chrome、Firefox 等浏览器都可以访问。
- Steam 叠加层刚启动时会短暂重试，避免把“仍在加载”误判为不可用。
- 日志会记录实际使用的打开方式和访问地址，方便排查页面没有自动打开的情况。
